...
public function chkLogout($username,$passwd){
	...
	function selectReporting($roleId){
		$return_results_top = array();
		$query = "SELECT um.userId,um.emp_code,um.userName,rm.roleName FROM tbl_user_master as um LEFT JOIN tbl_role_master as rm ON um.roleId = rm.roleId LEFT JOIN tbl_user_hierarchy as uh ON um.userId = uh.userId  WHERE rm.roleId < ".$roleId." ORDER BY um.userId ASC";
		$sql_query = mysql_query($query, $this->dbh);	
		$data_result = array();
	...
	function select_city($stateid){
		$return_results_top = array();
	$query = "SELECT distinct(city_name),city_id FROM tbl_city where state_id = $stateid order by city_name ASC";
	$sqlQuery = mysql_query($query, $this->dbh);
		$data_result = array();
		while($result = mysql_fetch_array($sqlQuery)){
